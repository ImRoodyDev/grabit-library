...
// providers/media/en/xpass/index.ts
var index_exports = {};
...
// providers/media/en/xpass/config.ts
var config = {
  scheme: "xpass",
  name: "Xpass",
  language: "en",
...
// providers/media/en/xpass/stream.ts
var MAX_SOURCES = 8;
  ...
  _getStreams = _asyncToGenerator(function* (requester, ctx) {
    ...
    try {
      ...
      const resolved = yield page.evaluate(/*#__PURE__*/function () {
        var _ref7 = _asyncToGenerator(function* (maxSources) {
          const sleep = ms => new Promise(r => setTimeout(r, ms));
          let dataUrl = "";
          for (let i = 0; i < 12 && !dataUrl; i++) {
            dataUrl = performance.getEntriesByType("resource").map(e => e.name).find(u => /\/data\/(movie|tv)\//i.test(u)) || "";
            if (!dataUrl) yield sleep(1e3);
          }
          if (!dataUrl) return {
            error: "no-data-url"
          };
          ...
          for (const s of sources.slice(0, maxSources)) {
            if (!s?.url) continue;
            try {
              const pj = yield (yield fetch(s.url)).json();
              const files = [];
              findFiles(pj, files);
      ...
      ctx.log.error(`[xpass] Browser flow failed: ${error.message}`);
      return [];
    } finally {
...
var index_default = defineProviderModule(PROVIDER, manifest_default.providers["xpass"], {
  getStreams
});