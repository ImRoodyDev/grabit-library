...
// providers/media/en/9filmyzilla/index.ts
var index_exports = {};
...
var _tldts = __toESM(require_cjs(), 1);
...
// providers/media/en/9filmyzilla/config.ts
var config = {
  scheme: "9filmyzilla",
  name: "9filmyzilla",
  language: ["en", "es"],
...
// providers/extractors/cherryupns.ts
var CHERRY_UPNS_KEY = Buffer.from("kiemtienmua911ca", "utf-8");
var CHERRY_UPNS_IV = Buffer.from("1234567890oiuytr", "utf-8");
function decryptCherryUpnsResponse(hexCiphertext) {
...
async function extractCherryUpnsStream(embedURL, ctx, requester, meta = {}) {
  ...
  const encryptedResponse = await ctx.xhr.fetchResponse(downloadURL, fetchOpts, requester);
  const decrypted = decryptCherryUpnsResponse(encryptedResponse);
  const sourceUrls = [decrypted.source, decrypted.cf].filter(Boolean);
...
// providers/media/en/9filmyzilla/stream.ts
var TITLE_MATCH_THRESHOLD = 85;
var REMOVE_PARENTHESES_REGEX = /\s*\([^)]*\)\s*/g;
var DETECT_SEASON_REGEX = /\bseason\s+(\d+)\b/i;
var DETECT_EPISODE_REGEX = /\bepisode\s+(\d+)\b/i;
async function getStreams(requester, ctx) {
  ...
  if (requester.media.type === "serie" && takeMovieApproach) {
    ...
    multiaudioServer = episodes.find((server) => server.name.includes(PROVIDER.createPatternString("s{season:2} e{episode:2}", requester.media))) ?? null;
    ...
    const episodePath = PROVIDER.createPatternString("episodes/{query:string}-season-{season:1}-episode-{episode:1}", requester.media, {
      query: bestResourceURL.pathname.split("/").filter((segment) => segment.trim() !== "").slice(-1)[0]
    });
  ...
  const iframeResponse = await ctx.xhr.fetchResponse(
    iframeRequestURL,
    iframeRequestOpts,
    requester
  );
...
function extractServers(page) {
  ...
  return $servers.map(($server) => {
    ...
    const dataType = page.$($server).attr("data-type");
    const flagURL = page.$($server).find("img").attr("src");
    const languageIso = flagURL ? flagURL.split("/").pop()?.split(".").shift() : void 0;
    return {
      name: serverName,
      type: dataType,
      post: dataPost,
  ...
  }).filter((server) => !server.name.includes("trailer") && !!server.post && !!server.nume).sort(
    (a, b) => Object.entries(b).reduce((acc, [key, value]) => acc + Number(!!value), 0) - Object.entries(a).reduce((acc, [key, value]) => acc + Number(!!value), 0)
  );
}
function selectBestResult(resultsPage, results, media) {
  if (media.type === "serie") {
    return results.map(($result) => ({ $result, scores: tvResultsScores(resultsPage, $result, media) })).filter(({ scores }) => {
  ...
  if (media.type === "movie") {
    return results.map(($result) => ({ $result, scores: movieResultScores(resultsPage, $result, media) })).filter(({ scores }) => scores.titleMatch && scores.isMovie).sort((a, b) => b.scores.titleScore - b.scores.yearDiff - (a.scores.titleScore - a.scores.yearDiff)).at(0) ?? null;
  }
  return null;
}
function movieResultScores(resultsPage, $result, media) {
  const type = resultsPage.$($result).find("article .thumbnail span").text().trim().toLowerCase();
...
function tvResultsScores(resultsPage, $result, media) {
  const type = resultsPage.$($result).find("article .thumbnail span").text().trim().toLowerCase();
  const title = resultsPage.$($result).find(locators.$result_title).text().replace(REMOVE_PARENTHESES_REGEX, "").trim();
  const href = resultsPage.$($result).find(locators.$result_entry).attr("href");
...
// providers/media/en/9filmyzilla/index.ts
var index_default = defineProviderModule(PROVIDER, manifest_default.providers["9filmyzilla"], {
  getStreams
});
