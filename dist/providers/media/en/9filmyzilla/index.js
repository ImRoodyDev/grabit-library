...
function extractCherryUpnsStream(_x14, _x15, _x16) {
  return _extractCherryUpnsStream.apply(this, arguments);
}
function _extractCherryUpnsStream() {
  _extractCherryUpnsStream = _asyncToGenerator(function* (embedURL, ctx, requester, meta = {}) {
    ...
    const encryptedResponse = yield ctx.xhr.fetchResponse(downloadURL, fetchOpts, requester);
...
${sourceUrls.join("\n")}`);
    return sourceUrls.map((url, index) => ({
      ...
      xhr: {
        ...
        flags: !!decrypted.cf && decrypted.cf.includes(url) ? ["CORS_BLOCKED"] : [],
        headers: {
          ...fetchOpts.headers
        }
      }
  ...
  _getStreams = _asyncToGenerator(function* (requester, ctx) {
    ...
    for (let i = 0; i < searchURLs.length; i++) {
      try {
        ...
        const resultsPage = yield ctx.cheerio.load(resourceURL, pageRequestOpt, ctx.xhr);
    ...
    const bestResourcePage = yield ctx.cheerio.load(bestResourceURL, pageRequestOpt, ctx.xhr);
    ...
    if (requester.media.type === "serie" && takeMovieApproach) {
      ...
      const episodePage = yield ctx.cheerio.load(episodeURL, pageRequestOpt, ctx.xhr);
    ...
    const iframeResponse = yield ctx.xhr.fetchResponse(iframeRequestURL, iframeRequestOpts, requester);
    ...
    if (iframeResponse.embed_url.includes("cherry.upns.online")) {
      return yield extractCherryUpnsStream(new URL(iframeResponse.embed_url), ctx, requester);
    } else {
...
function extractServers(page) {
  ...
  }).filter(server => !server.name.includes("trailer") && !!server.post && !!server.nume).sort((a, b) => Object.entries(b).reduce((acc, [key, value]) => acc + Number(!!value), 0) - Object.entries(a).reduce((acc, [key, value]) => acc + Number(!!value), 0));
}
function selectBestResult(resultsPage, results, media) {
  if (media.type === "serie") {
    return results.map($result => ({
      $result,
      scores: tvResultsScores(resultsPage, $result, media)
    })).filter(({
      scores
    }) => {
      const {
  ...
  if (media.type === "movie") {
    return results.map($result => ({
      $result,
      scores: movieResultScores(resultsPage, $result, media)
    })).filter(({
      scores
    }) => scores.titleMatch && scores.isMovie).sort((a, b) => b.scores.titleScore - b.scores.yearDiff - (a.scores.titleScore - a.scores.yearDiff)).at(0) ?? null;
  }
  return null;
}
