...
function _getStreams() {
  _getStreams = _asyncToGenerator(function* (requester, ctx) {
    ...
    pageRequestOpt.extraHeaders.cookie = (yield getKeyCookies(pageRequestOpt, requester, ctx)) ?? "";
    ...
    ctx.log.info(`Best match found: ${bestResult.inputs.title} (${bestResult.inputs.year}) with score ${bestResult.score} href: ${bestResult.inputs.entry}`);
    ...
    if (bestResult.inputs.entry.includes("/movie/")) {
      ...
      servers = yield getServers(bestResourceURL, pageRequestOpt.extraHeaders.cookie, requester, ctx);
    } else if (bestResult.inputs.entry.includes("/tv/") && requester.media.type === "serie") {
      const episodeURL = yield getEpisodeURL(bestResourceURL, pageRequestOpt, requester, ctx);
      if (!episodeURL) return [];
      pageRequestOpt.extraHeaders.Referer = episodeURL.href;
      servers = yield getServers(episodeURL, pageRequestOpt.extraHeaders.cookie, requester, ctx);
    ...
    for (const server of servers) {
      try {
        ...
        if (server.name.includes("mixdrop")) {
          const source = yield extractMixdropStream(new URL(server.url), extractOpts, ctx, {
...
function _getKeyCookies() {
  _getKeyCookies = _asyncToGenerator(function* (pageRequestOpt, requester, ctx) {
    const resourceURL = new URL("home", PROVIDER.config.baseUrl);
    ctx.log.info(`Fetching cookies from: ${resourceURL.href}`);
    const response = yield ctx.xhr.fetch(resourceURL, {
      attachUserAgent: true,
      clean: true,
      method: "GET",
      headers: {
...
function _getEpisodeURL() {
  _getEpisodeURL = _asyncToGenerator(function* (showURL, pageRequestOpt, requester, ctx) {
    if (requester.media.type !== "serie") return null;
    const media = requester.media;
    const {
      type,
      id,
      name = encodeURI(media.title, "form-uri")
    } = extractResultIdFromURL(showURL) || {};
    if (!type || !id) {
      ctx.log.warn("Could not extract type and id from the URL.");
      return null;
    }
    const showPage = yield ctx.cheerio.load(showURL, pageRequestOpt, ctx.xhr);
    ...
    ctx.log.info(`Found matching episode:
...
function _getServers() {
  _getServers = _asyncToGenerator(function* (targetURL, cookies, requester, ctx) {
    const {
      type,
      id
    } = extractResultIdFromURL(targetURL) || {};
    if (!type || !id) {
      ctx.log.warn("Could not extract type and id from the URL.");
      return [];
    }
    ...
    const serverResponse = yield ctx.xhr.fetchResponse(serverRequestURL, apiOpts, requester);
    ...
    const validServers = rawServers.filter(server => [, /*'filemoon'*/
    "mixdrop"].includes(server.name.toLowerCase()) && server.key);
    ...
    for (const server of validServers.slice(0, 2)) {
      try {
        ...
        try {
          ...
          const streamingLink = yield ctx.xhr.fetchResponse(serverURL, resolveOpts, requester);
          resolved.push({
