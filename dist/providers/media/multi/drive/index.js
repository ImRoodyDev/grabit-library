...
// providers/media/multi/drive/config.ts
var config = {
  scheme: "drive",
  name: "MoviesDrive",
  language: ["hi", "en"],
...
function fetchTextWithCloudflareFallback2(_x40, _x41, _x42, _x43) {
  return _fetchTextWithCloudflareFallback2.apply(this, arguments);
}
function _fetchTextWithCloudflareFallback2() {
  _fetchTextWithCloudflareFallback2 = _asyncToGenerator(function* (target, requester, ctx, cookieJar) {
    const headers = {
    ...
    try {
      const res = yield ctx.xhr.fetch(target, {
        method: "GET",
        attachUserAgent: true,
        clean: true,
        headers
      }, requester);
      if (res.status !== 403) return yield res.text().catch(() => null);
      ctx.log.warn(`[gdflix] Cloudflare 403 for ${target.href}, using browser session.`);
    } catch (error) {
      ctx.log.warn(`[gdflix] xhr fetch failed for ${target.href} (${error.message}), using browser.`);
    }
    try {
      const solved = yield ctx.solveChallenge(target, requester, {
        waitForCookie: "cf_clearance"
      ...
      ctx.log.error(`[gdflix] Challenge solve failed for ${target.href}: ${error.message}`);
      return null;
    }
  });
  return _fetchTextWithCloudflareFallback2.apply(this, arguments);
}
function extractGdflixStreams(_x44, _x45, _x46, _x47) {
  return _extractGdflixStreams.apply(this, arguments);
}
function _extractGdflixStreams() {
  _extractGdflixStreams = _asyncToGenerator(function* (link, requester, ctx, meta) {
    const startURL = typeof link === "string" ? new URL(link) : link;
    const baseUrl = startURL.origin;
    const cookieJar = {
      cookie: ""
    ...
    let html = yield fetchTextWithCloudflareFallback2(startURL, requester, ctx, cookieJar);
    if (!html) {
    ...
    if (replaced) {
      const next = yield fetchTextWithCloudflareFallback2(new URL(replaced, baseUrl), requester, ctx, cookieJar);
    ...
    try {
      ...
      if (resumeHref && !resumeHref.includes("indexbot")) {
        ...
        const resumeHtml = yield fetchTextWithCloudflareFallback2(resumeURL, requester, ctx, cookieJar);
        if (resumeHtml) {
          const link2 = ctx.cheerio.$load(resumeHtml)(".btn-success").attr("href");
          if (link2) raw.push({
            server: "ResumeCloud",
            url: new URL(link2, resumeURL.origin).href
          });
        }
      }
    } catch (error) {
      ctx.log.debug(`[gdflix] ResumeCloud failed: ${error.message}`);
    }
    try {
      const seed = $(".btn-danger").attr("href") || "";
      if (seed && !seed.includes("?url=")) {
        const res = yield ctx.xhr.fetch(new URL(seed, baseUrl), {
          method: "HEAD",
          attachUserAgent: true,
          clean: true,
          headers: {
...
function titleTokens(title) {
  return cleanTitle(title).toLowerCase().split(/\s+/).filter(t => t.length >= 3);
}
...
function _getStreams() {
  _getStreams = _asyncToGenerator(function* (requester, ctx) {
    ...
    const results = [];
    const tokens = titleTokens(media.title);
    for (const cand of candidates.slice(0, MAX_CANDIDATES)) {
      try {
        const resolved = yield resolveDriveLink(cand.link, requester, ctx, pageOpt);
        if (!resolved) {
        ...
        const sources = resolved.kind === "gdflix" ? yield extractGdflixStreams(resolved.url, requester, ctx, meta) : yield extractHubcloudStreams(resolved.url, requester, ctx, {
          ...meta,
          matchTokens: tokens
        });
        results.push(...sources);
      } catch (error) {
        ctx.log.error(`[drive] Candidate resolution failed (${cand.link}): ${error.message}`);
      }
    }
...
function searchPosts(_x50, _x51, _x52) {
...
function _searchPosts() {
  _searchPosts = _asyncToGenerator(function* (url, requester, ctx) {
    ...
    try {
      ...
      ctx.log.warn(`[drive] Search failed for ${url.href}: ${error.message}`);
      return [];
    }
  });
  return _searchPosts.apply(this, arguments);
}
function getCandidateLinks(_x53, _x54, _x55, _x56) {
...
function _getCandidateLinks() {
  _getCandidateLinks = _asyncToGenerator(function* (link, requester, ctx, pageOpt) {
    ...
    const media = requester.media;
    const candidates = [];
    const isDownloadHost = h2 => /hubcloud|gdflix|hubdrive|search-recover|\/archives\//i.test(h2) && !/\/(category|genre|tag)\/|[?&]s=/i.test(h2);
    $("h5 a, a").each((_, el) => {
      const text = $(el).text();
      ...
      if (media.type === "serie") {
        ...
        if (season != null && season !== Number(media.season)) return;
      }
      candidates.push({
        quality,
        link: href,
...
function resolveDriveLink(_x57, _x58, _x59, _x60) {
  return _resolveDriveLink.apply(this, arguments);
}
function _resolveDriveLink() {
  _resolveDriveLink = _asyncToGenerator(function* (link, requester, ctx, pageOpt) {
    let url = new URL(link, `${PROVIDER.config.baseUrl}/`).href;
    try {
      const {
        $
      } = yield ctx.cheerio.load(new URL(url), pageOpt, ctx.xhr);
      const host = $('a:contains("HubCloud")').attr("href") || $('a:contains("GDFlix"),a:contains("GDFLIX")').attr("href");
      if (host) url = new URL(host, url).href;
    } catch {}
    if (/gdflix/i.test(url)) return {
      kind: "gdflix",
      url
    };
    if (/hubcloud/i.test(url)) return {
      kind: "hubcloud",
      url
    };
    try {
      const text = yield getText(new URL(url), requester, ctx);
      let redirectUrl = text.match(/<meta\s+http-equiv="refresh"\s+content="[^"]*?;\s*url=([^"]+)"\s*\/?>/i)?.[1] || "";
      if (url.includes("/archives/")) {
        redirectUrl = text.match(/<a\s+[^>]*href="(https:\/\/hubcloud\.[^/]+\/[^"]+)"/i)?.[1] || redirectUrl;
      }
      if (!redirectUrl) return null;
      if (/gdflix/i.test(redirectUrl)) return {
        kind: "gdflix",
        url: redirectUrl
      };
      if (/hubcloud/i.test(redirectUrl)) return {
        kind: "hubcloud",
        url: redirectUrl
      };
      const {
        $
      } = yield ctx.cheerio.load(new URL(redirectUrl), pageOpt, ctx.xhr);
      const hubcloudLink = $(".fa-file-download").parent().attr("href") || "";
      const finalUrl = /hubcloud/i.test(hubcloudLink) ? hubcloudLink : redirectUrl;
      return {
        kind: /gdflix/i.test(finalUrl) ? "gdflix" : "hubcloud",
        url: finalUrl
      };
    } catch (error) {
      ctx.log.debug(`[drive] resolveDriveLink failed: ${error.message}`);
      return null;
    }
  });
  return _resolveDriveLink.apply(this, arguments);
}
function getText(_x61, _x62, _x63) {
  return _getText.apply(this, arguments);
} // providers/media/multi/drive/lazy.ts
function _getText() {
  _getText = _asyncToGenerator(function* (url, requester, ctx) {
...
function getLazyStreams(_x64, _x65) {
...
function resolveLazy(_x66, _x67, _x68) {
...
function _resolveLazy() {
  _resolveLazy = _asyncToGenerator(function* (id, ctx, requester) {
    ...
    try {
      const resolved = yield resolveDriveLink2(handle.link, requester, ctx, pageOpt);
      ...
      const sources = resolved.kind === "gdflix" ? yield extractGdflixStreams(resolved.url, requester, ctx, meta) : yield extractHubcloudStreams(resolved.url, requester, ctx, {
        ...meta,
        matchTokens: titleTokens(requester.media.title)
      });
...
function searchPosts2(_x69, _x70, _x71) {
...
function getCandidateLinks2(_x72, _x73, _x74, _x75) {
...
function resolveDriveLink2(_x76, _x77, _x78, _x79) {
  return _resolveDriveLink2.apply(this, arguments);
}
function _resolveDriveLink2() {
  _resolveDriveLink2 = _asyncToGenerator(function* (link, requester, ctx, pageOpt) {
...
// providers/media/multi/drive/index.ts
var index_default = defineProviderModule(PROVIDER, manifest_default.providers["drive"], {
  getStreams,
  getLazyStreams,
  resolveLazy
});