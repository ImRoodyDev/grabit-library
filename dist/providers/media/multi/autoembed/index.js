  ...
  _getStreams = _asyncToGenerator(function* (requester, ctx) {
    ...
    for (const server of servers) {
      try {
        ...
        const apiResponse = yield ctx.xhr.fetchResponse(apiURL, apiOpts, requester);
        ctx.log.debug(`API Response: ${JSON.stringify(apiResponse)}`);
        const decryptedData = yield decryptData(apiResponse.data);
...
function _decryptData() {
  _decryptData = _asyncToGenerator(function* (data) {
    ...
    const key = Crypto.pbkdf2Sync(p250.key, salt, p250.iterations, 32,
