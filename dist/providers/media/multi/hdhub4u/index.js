...
// providers/media/multi/hdhub4u/index.ts
var index_exports = {};
...
// providers/media/multi/hdhub4u/config.ts
var config = {
  scheme: "hdhub4u",
  name: "HdHub4u",
  language: ["hi", "en"],
...
function resolveToHubcloud(_x40, _x41, _x42, _x43) {
  return _resolveToHubcloud.apply(this, arguments);
}
function _resolveToHubcloud() {
  _resolveToHubcloud = _asyncToGenerator(function* (link, requester, ctx, pageOpt) {
    if (link.includes("hubcloud") || link.includes("/drive/")) return link;
    if (link.includes("hubdrive")) {
      const {
        $
      } = yield ctx.cheerio.load(new URL(link), pageOpt, ctx.xhr);
      return $(".btn.btn-primary.btn-user.btn-success1.m-1").attr("href") || $(".btn.btn-primary.btn-user.btn-success1").attr("href") || link;
    }
    try {
      const text = yield getText(new URL(link), requester, ctx);
      ...
      const blogLink = yield getRedirectLink(decodedLink, requester, ctx);
      if (blogLink.includes("hubcloud") || blogLink.includes("/drive/")) return blogLink;
      const blogText = yield getText(new URL(blogLink), requester, ctx);
      const $ = ctx.cheerio.$load(blogText);
      const driveLinks = [];
      $('a[href*="hubcloud"][href*="/drive/"]').each((_, el) => {
        const href = $(el).attr("href");
        if (!href) return;
        const t = ($(el).text() + $(el).parent().text()).toLowerCase();
        driveLinks.push({
          quality: detectQuality(t),
          link: href
        });
      });
      const order = {
      ...
      let hubdriveLink = driveLinks[0]?.link || $('h3:contains("1080p")').find("a").attr("href") || blogText.match(/href="(https:\/\/hubcloud\.[^/]+\/drive\/[^"]+)"/)?.[1] || "";
      if (hubdriveLink.includes("hubdrive")) {
        const {
          $: $$
        } = yield ctx.cheerio.load(new URL(hubdriveLink), pageOpt, ctx.xhr);
        hubdriveLink = $$(".btn.btn-primary.btn-user.btn-success1.m-1").attr("href") || $$(".btn.btn-primary.btn-user.btn-success1").attr("href") || hubdriveLink;
      }
      if (!hubdriveLink) return null;
      try {
        const finalText = yield getText(new URL(hubdriveLink), requester, ctx);
      ...
      ctx.log.debug(`[hubchain] Obfuscated resolution failed: ${error.message}`);
      return null;
    }
  });
  return _resolveToHubcloud.apply(this, arguments);
}
function getRedirectLink(_x44, _x45, _x46) {
  return _getRedirectLink.apply(this, arguments);
}
function _getRedirectLink() {
  _getRedirectLink = _asyncToGenerator(function* (link, requester, ctx) {
    try {
      const resText = yield getText(new URL(link), requester, ctx);
      ...
      if (waitS > 0) yield sleep(waitS * 1e3);
      return blogLink || link;
    } catch (error) {
      ctx.log.debug(`[hubchain] getRedirectLink failed: ${error.message}`);
      return link;
    }
  });
  return _getRedirectLink.apply(this, arguments);
}
function getText(_x47, _x48, _x49) {
...
var encode = value => btoa(String(value));
var decode = value => value === void 0 ? "" : atob(String(value));
function pen(value) {
...
// providers/media/multi/hdhub4u/stream.ts
var HDB_HEADERS = {
  cookie: "xla=s4t",
  Referer: "https://google.com"
};
...
function _getStreams() {
  _getStreams = _asyncToGenerator(function* (requester, ctx) {
    ...
    for (const cand of candidates.slice(0, MAX_CANDIDATES)) {
      try {
        const hubcloudLink = yield resolveToHubcloud(cand.link, requester, ctx, pageOpt);
        if (!hubcloudLink) {
        ...
        const sources = yield extractHubcloudStreams(hubcloudLink, requester, ctx, {
          ...
          language,
          matchTokens: titleTokens(media.title)
        });
        results.push(...sources);
      } catch (error) {
        ctx.log.error(`[hdhub4u] Candidate resolution failed (${cand.link}): ${error.message}`);
      }
    }
...
function searchPosts(_x52, _x53, _x54) {
...
function _searchPosts() {
  _searchPosts = _asyncToGenerator(function* (url, requester, ctx) {
    ...
    try {
      ...
      ctx.log.warn(`[hdhub4u] Search API failed for ${url.href}: ${error.message}`);
      return [];
    }
  });
  return _searchPosts.apply(this, arguments);
}
function getCandidateLinks(_x55, _x56, _x57, _x58) {
  return _getCandidateLinks.apply(this, arguments);
} // providers/media/multi/hdhub4u/lazy.ts
function _getCandidateLinks() {
  _getCandidateLinks = _asyncToGenerator(function* (link, requester, ctx, pageOpt) {
    ...
    if (media.type === "serie") {
      ...
      container.find("strong").each((_, el) => {
        ...
        for (const sibling of heading.nextAll().toArray()) {
          ...
          $sib.find('a[href*="hubdrive"],a[href*="hubcloud"],a:contains("Drive")').each((__, a) => {
      ...
      if (candidates.length === 0) {
        container.find('a:contains("EPiSODE"),a:contains("Episode")').each((_, el) => {
    ...
    if (candidates.length === 0 && media.type === "movie") {
      container.find('a:contains("480"),a:contains("720"),a:contains("1080"),a:contains("2160"),a:contains("4K")').each((_, el) => {
        const text = $(el).text();
        const href = $(el).attr("href");
...
function getLazyStreams(_x59, _x60) {
...
function _getLazyStreams() {
  _getLazyStreams = _asyncToGenerator(function* (requester, ctx) {
    ...
    if (!best) return [];
    const candidates = yield candidatesFor(best.post.link, requester, ctx, pageOpt);
...
function resolveLazy(_x61, _x62, _x63) {
...
function _resolveLazy() {
  _resolveLazy = _asyncToGenerator(function* (id, ctx, requester) {
    ...
    try {
      ...
      const link = yield resolveToHubcloud(handle.link, requester, ctx, pageOpt);
      if (!link) return null;
      const sources = yield extractHubcloudStreams(link, requester, ctx, {
...
function search(_x64, _x65, _x66) {
...
function candidatesFor(_x67, _x68, _x69, _x70) {
  return _candidatesFor.apply(this, arguments);
}
function _candidatesFor() {
  _candidatesFor = _asyncToGenerator(function* (link, requester, ctx, pageOpt) {
    const {
      $
...
// providers/media/multi/hdhub4u/index.ts
var index_default = defineProviderModule(PROVIDER, manifest_default.providers["hdhub4u"], {
  getStreams,
  getLazyStreams,
  resolveLazy
});