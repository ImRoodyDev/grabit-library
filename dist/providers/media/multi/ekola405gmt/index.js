...
function _getStreams() {
  _getStreams = _asyncToGenerator(function* (requester, ctx) {
    ...
    const playerConfig = yield getPlayerConfig(iframeURL, ymCookies, requester, ctx);
    if (!playerConfig) return [];
    const sources = yield getStreamSources(iframeURL, playerConfig, ymCookies, requester, ctx);
    if (sources.length === 0) {
    ...
    const servers = yield resolveServers(sources, iframeURL, playerConfig, ymCookies, requester, ctx);
...
function _getPlayerConfig() {
  _getPlayerConfig = _asyncToGenerator(function* (iframeURL, ymCookies, requester, ctx) {
    const rootIframe = yield ctx.cheerio.load(iframeURL, {
      ...requester,
      extraHeaders: {
...
function _getStreamSources() {
  _getStreamSources = _asyncToGenerator(function* (iframeURL, playerConfig, ymCookies, requester, ctx) {
    const fileURL = new URL(playerConfig.file, iframeURL.origin);
    ctx.log.info(`Constructed file URL from player config: ${fileURL}`);
    const fileResponse = yield ctx.xhr.fetch(fileURL, {
      method: "POST",
      clean: true,
      attachUserAgent: true,
      headers: {
    ...
    const file = yield fileResponse.json().then(data => Array.isArray(data) ? data.flat() : []);
...
function _resolveServers() {
  _resolveServers = _asyncToGenerator(function* (sources, iframeURL, playerConfig, ymCookies, requester, ctx) {
    const fileURL = new URL(playerConfig.file, iframeURL.origin);
    ...
    for (const source of sources) {
      try {
        const server = yield resolveStreamURL(source, fileURL, fileDir, fileExtension, postHeaders, m3u8Headers, requester, ctx);
...
function _resolveStreamURL() {
  _resolveStreamURL = _asyncToGenerator(function* (source, fileURL, fileDir, fileExtension, postHeaders, m3u8Headers, requester, ctx) {
    ...
    const fileTextResponse = yield ctx.xhr.fetch(targetURL, {
      method: "POST",
      clean: true,
      attachUserAgent: true,
      headers: postHeaders
    }, requester);
    const m3u8URL = yield fileTextResponse.text();
    const m3u8Opts = {
      method: "GET",
      clean: true,
      attachUserAgent: true,
    ...
    const m3u8FileResponse = yield ctx.xhr.fetch(m3u8URL, m3u8Opts, requester);
