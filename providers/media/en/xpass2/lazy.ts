...
export async function resolveLazy(
	...
	try {
		const response = await ctx.xhr.fetch(
			new URL(entry.url, base),
			{ method: 'GET', attachUserAgent: true, clean: true, headers: session.headers },
			requester,
		);
		...
		return {
			...
			language: 'en',
			format: 'm3u8',
			xhr: { flags: ['CORS_BLOCKED'], headers: { Referer: base.origin + '/', Origin: base.origin } },
		};
	} catch {
		return null;
	}
...
async function getSession(
	...
	requester: ScrapeRequester,
	ctx: ProviderContext,
): Promise<{ dataUrl: string; headers: Record<string, string> } | null> {
	const base = new URL(PROVIDER.config.baseUrl);
	try {
		const response = await ctx.xhr.fetch(
			embedUrl,
			{
				...
				clean: true,
				headers: { Referer: base.origin + '/', 'accept-language': 'en-US,en;q=0.9' },
			},
			requester,
		);
		...
		return {
			...
			headers: {
				...
				...(createCookiesFromSet(response.headers as any)
					? { cookie: createCookiesFromSet(response.headers as any) }
					: {}),
			},
		};
	} catch {
...
function isLocalUrl(value: string, base: URL): boolean {
	try {
...
function findFiles(value: any, out: { file: string; label?: string }[]): void {
	if (Array.isArray(value)) return value.forEach((item) => findFiles(item, out));
	if (value && typeof value === 'object') {
		if (typeof value.file === 'string' && /^https?:\/\//.test(value.file))
			out.push({ file: value.file, label: value.label });
		for (const key of Object.keys(value)) if (key !== 'file') findFiles(value[key], out);
	}
}
