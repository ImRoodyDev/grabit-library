 ...
 * (`ctx.xhr`) and only falls back to `ctx.puppeteer` when the page is Cloudflare-gated.
 * It works because the signed `/data/<type>/<tmdb>?…&sig=…` URL is server-rendered
 * into the embed HTML (`var dataUrl="…"`), so no client-side JS is required.
 */
