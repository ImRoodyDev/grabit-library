...
export async function getLazyStreams(requester: ScrapeRequester, ctx: ProviderContext): Promise<InternalMediaSource[]> {
	...
	let posts: Post[] = [];
	const imdb = (requester.media as any).imdbId
		? new URL(PROVIDER.createPatternString('/?s={imdb:string}', requester.media), PROVIDER.config.baseUrl)
		: undefined;
...
export async function resolveLazy(
	...
	try {
		const url = await resolveLink(handle.link, requester, ctx);
		if (!url) return null;
		const sources = await extractHubcloudStreams(url, requester, ctx, {
...
async function search(url: URL, requester: ScrapeRequester, ctx: ProviderContext): Promise<Post[]> {
	try {
		const { $ } = await ctx.cheerio.load(url, { ...requester, followRedirects: true, extraHeaders: {} }, ctx.xhr);
		const out: Post[] = [];
		$('article,.pstr_box,.result-item,.post,.item,.thumbnail,.movie-item,.ml-item').each((_: number, element: any) => {
			const href = $(element).find('a[href]').first().attr('href') || '';
			const title = (
				$(element).find('.entry-title,.post-title,.title,h2,h3').first().text() || $(element).find('a').first().text()
			).trim();
			if (href && title) out.push({ title, link: href, image: '' });
		});
		return out;
	} catch {
		return [];
...
async function resolveLink(link: string, requester: ScrapeRequester, ctx: ProviderContext): Promise<string | null> {
	...
	try {
		...
		const data = await ctx.xhr.fetchResponse<any>(
			new URL(`/api/s/${id}/hubcloud`, url.origin),
