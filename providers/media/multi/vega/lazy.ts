...
async function searchPosts(url: URL, requester: ScrapeRequester, ctx: ProviderContext): Promise<SearchPost[]> {
	try {
		const json = await ctx.xhr.fetchResponse<{ hits?: Array<{ document?: any }> }>(url, { method: 'GET', attachUserAgent: true, clean: true, headers: { ...HEADERS, Referer: PROVIDER.config.baseUrl + '/' } }, requester);
		return (Array.isArray(json?.hits) ? json.hits : []).map((hit) => hit.document || {}).map((doc) => {
			...
			return { title: String(doc.post_title || '').replace(/Download/gi, '').trim(), link: `${postUrl.pathname}${postUrl.search}${postUrl.hash}`, image: String(doc.post_thumbnail || '') };
	...
	if (requester.media.type === 'serie') {
		...
		$('h3').each((_: number, heading: any) => {
			...
			while (node.length && !node.is('h3') && guard++ < 8) {
				const href = node.find('a').filter((__: number, anchor: any) => /episode/i.test($(anchor).text())).first().attr('href');
		...
		for (const page of unique(episodePages.map((item) => item.href)).slice(0, 3).map((href) => episodePages.find((item) => item.href === href)!)) {
			...
			if (episode && isAllowedVegaLink(episode.link)) candidates.push({ quality: page.quality, link: episode.link, label: `E${media.episode}` });
		}
	} else {
...
async function getEpisodeList(link: string, requester: ScrapeRequester, ctx: ProviderContext, pageOpt: CheerioLoadRequest): Promise<{ title: string; link: string }[]> {
	try {
		let target = link;
		if (target.includes('url=')) {
			try { target = atob(target.split('url=')[1]!); } catch { return []; }
		}
		...
		container.find('h3,h4').each((_: number, element: any) => {
			...
			const hrefs = $(element).nextUntil('h3,h4').find('a').map((__: number, anchor: any) => $(anchor).attr('href')).get().filter((href: string) => href && href !== '#');
			const href = hrefs.find((value: string) => /vcloud|hubcloud|v-cloud|\/drive\/|hubdrive|cloud\./i.test(value)) || hrefs.find((value: string) => /nexdrive/i.test(value)) || hrefs[0];
...
async function resolveVegaLink(link: string, requester: ScrapeRequester, ctx: ProviderContext): Promise<string | null> {
	...
	try {
		const response = await ctx.xhr.fetch(new URL(link, PROVIDER.config.baseUrl), { method: 'GET', attachUserAgent: true, clean: true, headers: { ...HEADERS } }, requester);
...
function episodeNumberOf(title: string): number | null { const match = title.match(/episodes?\s*:?\s*(\d+)/i) || title.match(/\be\s*p?\s*(\d+)\b/i) || title.match(/\be(\d+)\b/i); return match ? Number(match[1]) : null; }
function unique<T>(items: T[]): T[] { return [...new Set(items)]; }
function uniqueCandidates(items: Candidate[]): Candidate[] { return items.filter((item, index) => items.findIndex((other) => other.link === item.link) === index); }
function qualityRank(value: string): number { return ({ '4k': 4, '2160p': 4, '1080p': 3, '720p': 2, '480p': 1 } as Record<string, number>)[value.toLowerCase()] || 0; }
