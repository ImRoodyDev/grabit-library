		...
		lazy: {
			id: encodeURIComponent(
				JSON.stringify({
					...
					playerFile: config.file,
					playerKey: config.key,
					source,
				} satisfies Handle),
			),
			label: source.title,
...
export async function resolveLazy(
	...
	const target = new URL(
		attachExtension(
			handle.source.end_tag || extension,
			pathJoin(dir, handle.source.file.startsWith('~') ? handle.source.file.slice(1) : handle.source.file),
		),
		iframe.origin,
	);
	try {
		const raw = await ctx.xhr
			.fetch(target, { method: 'POST', clean: true, attachUserAgent: true, headers: postHeaders }, requester)
			.then((response) => response.text());
		const playlist = await ctx.xhr.fetch(
			raw,
			{
				method: 'GET',
				clean: true,
				attachUserAgent: true,
				headers: { ...HEADERS, Referer: iframe.origin + '/', Origin: iframe.origin },
			},
			requester,
		);
		...
		return {
			...
			xhr: { flags: [], headers: { Referer: iframe.origin + '/', Origin: iframe.origin } },
		};
	} catch {
		return null;
	}
...
async function getConfig(
	...
	try {
		const page = await ctx.cheerio.load(
			url,
			{ ...requester, extraHeaders: { ...HEADERS, cookie: cookies, Referer: 'https://vegamovies.ad/' } },
			ctx.xhr,
		);
		const script = page.$('script:contains("HDVBPlayer")').html() || '';
		return (
			script.includes('HDVBPlayer({')
				? extractContructorJSONArguments(script)
				: extractVariableByJSONKey(script, ['file', 'key', 'href'])
		) as Config | null;
	} catch {
		return null;
	}
...
async function getSources(
	...
	try {
		const response = await ctx.xhr.fetch(
			new URL(config.file, url.origin),
			{
				method: 'POST',
				clean: true,
				attachUserAgent: true,
		...
		const data = await response.json().then((value) => (Array.isArray(value) ? value.flat() : []));
		const sources: any[] = requester.media.type === 'movie' ? data : [];
		if (requester.media.type === 'serie') {
			const id = `${(requester.media as any).season}-${(requester.media as any).episode}`;
			const season = data.find((item: any) => item.folder?.some((episode: any) => episode.id?.trim() === id));
			const episode = season?.folder?.find((item: any) => item.id?.trim() === id);
		...
		return sources
			...
			.map((source: any) => ({
				...
				title: source.title,
				language: ISO6391.getCode(String(source.title).split(' ').shift() || '') || 'unknown',
			}));
	} catch {
		return [];
	}
}
function makeCookies(): string {
	const now = Math.floor(Date.now() / 1000);
	return `_ym_uid=${now}${Math.floor(Math.random() * 1e9)
		.toString()
		.padStart(9, '0')}; _ym_d=${now}; _ym_isad=2`;
}
