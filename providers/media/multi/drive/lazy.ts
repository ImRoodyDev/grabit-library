	...
	try {
		...
		const sources =
			resolved.kind === 'gdflix'
				...
				: await extractHubcloudStreams(resolved.url, requester, ctx, {
						...meta,
...
async function searchPosts(url: URL, requester: ScrapeRequester, ctx: ProviderContext): Promise<SearchPost[]> {
	try {
		const json = await ctx.xhr.fetchResponse<{ hits?: Array<{ document?: any }> }>(
			url,
			{ method: 'GET', attachUserAgent: true, clean: true, headers: { Referer: PROVIDER.config.baseUrl + '/' } },
			requester,
		);
...
async function resolveDriveLink(
	...
	try {
		...
		const host =
			$('a:contains("HubCloud")').attr('href') || $('a:contains("GDFlix"),a:contains("GDFLIX")').attr('href');
