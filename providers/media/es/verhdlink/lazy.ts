...
export async function getLazyStreams(requester: ScrapeRequester, ctx: ProviderContext): Promise<InternalMediaSource[]> {
	...
	return embeds
		...
		.map((embed) => ({
			...
			language: 'es',
			lazy: {
				id: encodeURIComponent(JSON.stringify({ embed, sourcePage: pageUrl.href } satisfies Handle)),
				label: 'Spanish',
			},
			xhr: { flags: [], headers: {} },
		}));
...
export async function resolveLazy(
	...
	const sources = await dispatchEmbed(
		handle.embed,
		{ ...requester, extraHeaders: { Referer: PROVIDER.config.baseUrl + '/' } },
		ctx,
		'es',
	).catch(() => []);
...
async function getEmbeds(pageUrl: URL, requester: ScrapeRequester, ctx: ProviderContext): Promise<string[]> {
	...
	try {
		...
		const embeds: string[] = [];
		$('._player-mirrors [data-link], [data-link]').each((_: number, element: any) => {
