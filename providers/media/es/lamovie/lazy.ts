...
export async function getLazyStreams(requester: ScrapeRequester, ctx: ProviderContext): Promise<InternalMediaSource[]> {
	...
	return servers
		.filter((server) => isAllowedServer(server))
		.map((server) => ({
			fileName: `HLS ${server.quality}`,
			language: server.lang || 'es',
			lazy: {
				id: encodeURIComponent(JSON.stringify({ ...server, title: result.title })),
...
export async function resolveLazy(
	...
	try {
		if (handle.serverName === 'goodstream')
			return (await extractGoodstreamStreams(new URL(handle.url), requester, ctx, metadata))[0] ?? null;
		if (handle.serverName === 'vimeos')
			return (await extractVimeosStreams(new URL(handle.url), requester, ctx, metadata))[0] ?? null;
		if (handle.serverName === 'filemoon')
			return (await extractFilemoonStreams(new URL(handle.url), requester, ctx, metadata))[0] ?? null;
		return null;
	} catch {
		return null;
	}
...
async function findResult(requester: ScrapeRequester, ctx: ProviderContext): Promise<Result | null> {
	for (const search of PROVIDER.createResourceUrls(requester)) {
		try {
			...
			const response = await ctx.xhr.fetchResponse<any>(
				url,
				{ method: 'GET', attachUserAgent: true, clean: true, headers: { ...API_HEADERS, referer: url.origin + '/' } },
				requester,
			);
...
async function getEpisodeId(result: Result, requester: ScrapeRequester, ctx: ProviderContext): Promise<string | null> {
	...
	try {
		const data = await ctx.xhr.fetchResponse<any>(
			url,
			{ method: 'GET', attachUserAgent: true, clean: true, headers: API_HEADERS },
			requester,
		);
		return (
			data.data?.posts?.find(
				(episode: any) => episode.episode_number === media.episode && episode.season_number === media.season,
			)?._id ?? null
		);
	} catch {
		return null;
...
async function getServers(
	...
	try {
		const data = await ctx.xhr.fetchResponse<any>(
			url,
			{ method: 'GET', attachUserAgent: true, clean: true, headers: { ...API_HEADERS, Referer: result.entry } },
			requester,
		);
		return (data.data?.embeds ?? [])
			.map((embed: any) => {
				const raw = String(embed.lang || '')
					.toLowerCase()
					.split('-')[0];
		...
		posts
			.map((post) => {
				...
				return {
					...
					year,
					entry: new URL(
						PROVIDER.createPatternString('/{type:string}/{slug:string}/', media, { type, slug: post.slug }),
						PROVIDER.config.baseUrl,
					).href,
					score: Math.max(
						calculateMatchScore({ title, year }, media),
						calculateMatchScore({ title: post.title, year }, media),
					),
				};
			})
...
function isAllowedServer(server: Partial<Server>): boolean {
	try {
		...
		return (
			url.protocol === 'https:' &&
			['goodstream', 'vimeos', 'filemoon'].includes(server.serverName || '') &&
			new RegExp(server.serverName || 'x', 'i').test(url.hostname)
		);
	} catch {
		return false;
	}
